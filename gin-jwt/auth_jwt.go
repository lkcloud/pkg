...
type GinJWTMiddleware struct {
	...
	// ListTokenFunc get token list from func
	ListTokenFunc func() (*TokenList, error)
...
var (
	...
	// ErrKeyNotFound indicates can not found secret key by the given id
	ErrKeyNotFond = errors.New("key not found")
...
func NewTokenList() *TokenList {
	tl := TokenList{}
	tl.Tokens = make(map[string]string)
	return &tl
}
func (t *TokenList) Len() int {
	t.tMutex.RLock()
	defer t.tMutex.RUnlock()
	return len(t.Tokens)
}

func (t *TokenList) Key(id string) ([]byte, error) {
	t.tMutex.RLock()
	defer t.tMutex.RUnlock()
	if key, ok := t.Tokens[id]; ok {
		return []byte(key), nil
	}

	return nil, ErrKeyNotFond
}
...
func (mw *GinJWTMiddleware) syncToken() error {
	jwtToken = &JWTToken{}
	if tokenCache == nil {
		expiry := 300
		tokenCache = cache.New(time.Duration(expiry)*time.Second, 15*time.Second)
	}

	mw.tokenFromService(jwtToken)
	return nil
}
...
func (mw *GinJWTMiddleware) ParseToken(c *gin.Context) (*jwt.Token, error) {
	...
	return jwt.Parse(token, func(t *jwt.Token) (interface{}, error) {
		...
		return mw.getSecretKey(t.Claims.(jwt.MapClaims))
	})
}
...
func (mw *GinJWTMiddleware) getSecretKey(claims jwt.MapClaims) ([]byte, error) {
	if id, found := claims["jti"].(string); found {
		return mw.getSecretKeyFromCache(id)
	}

	return mw.Key, nil
}

func (mw *GinJWTMiddleware) getSecretKeyFromCache(id string) ([]byte, error) {
	tokenList, err := mw.tokenFromService(jwtToken)
	if err != nil {
		return nil, err
	}

	return tokenList.Key(id)
}

func (mw *GinJWTMiddleware) tokenFromService(jwtToken *JWTToken) (*TokenList, error) {
	if mw.ListTokenFunc == nil {
		return nil, fmt.Errorf("can not get token list, cache token not enable or list token function is nil")
	}

	doCacheRefresh := func() (*TokenList, error) {
		jwtToken.TokenRefreshInProgress = true
		defer func() { jwtToken.TokenRefreshInProgress = false }()
		data, err := mw.ListTokenFunc()
		if err != nil {
			return nil, err
		}
		jwtToken.HasRun = true
		// Set the cached value
		if data.Len() == 0 {
			if jwtToken.LastGoodTokenList == nil {
				jwtToken.LastGoodTokenList = NewTokenList()
			}

			return jwtToken.LastGoodTokenList, nil
		}

		tokenCache.Set("tokenmaps", data, cache.DefaultExpiration)
		// Stash it too
		jwtToken.LastGoodTokenList = data
		return data, nil
	}
	...
	cachedTokenData, found := tokenCache.Get("tokenmaps")
	if !found {
		if jwtToken.TokenRefreshInProgress {
			// Are we already refreshing the cache? skip and return last good conf
			return jwtToken.LastGoodTokenList, nil
		}
		// Refresh the jwtToken
		return doCacheRefresh()
	}

	return cachedTokenData.(*TokenList), nil
}
